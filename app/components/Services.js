  ...
  const services = [
    {
      ...
      badge: 'Most Popular', badgeRgb: 'var(--accent-rgb)',
    },
    {
      ...
      badge: 'Core Service', badgeRgb: '16,185,129',
    },
    {
      ...
      badge: 'High Impact', badgeRgb: '249,115,22',
    },
    {
      ...
      badge: 'Foundation', badgeRgb: '139,158,140',
    },
    {
      ...
      badge: 'New', badgeRgb: 'var(--accent-rgb)',
    },
    {
      ...
      badge: 'Advisory', badgeRgb: '139,158,140',
    },
  ];

  return (
    <section id="services" className="py-16 sm:py-24 md:py-28 relative">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-px h-20 pointer-events-none hidden sm:block" style={{ background: 'linear-gradient(to bottom, transparent, rgba(var(--accent-rgb),0.25), transparent)' }} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {services.map((s, i) => (
            <div key={i} className="card-glass group relative overflow-hidden">
              <div className="absolute top-0 left-0 right-0 h-px opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ background: 'linear-gradient(90deg, transparent, rgba(var(--accent-rgb),0.6), transparent)' }} />

              <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center mb-4 sm:mb-5 transition-all duration-300 group-hover:scale-110" style={{ background: 'rgba(var(--accent-rgb),0.08)', border: '1px solid rgba(var(--accent-rgb),0.15)', color: 'var(--accent)' }}>
                {s.icon}
              </div>

              <span className="inline-block px-2.5 py-0.5 text-[10px] font-bold tracking-wider rounded-full mb-3" style={{ background: `rgba(${s.badgeRgb},0.12)`, border: `1px solid rgba(${s.badgeRgb},0.3)`, color: `rgb(${s.badgeRgb})` }}>
                {s.badge.toUpperCase()}
              </span>

              <h3 className="text-base font-semibold mb-2.5 sm:mb-3 transition-colors duration-200" style={{ color: 'var(--text-primary)' }}>{s.title}</h3>
